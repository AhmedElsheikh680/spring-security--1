...
private static final SecretKey TOKEN_SECRET = Keys.secretKeyFor(SignatureAlgorithm.HS512);  // مفتاح قوي
...
//                .signWith(TOKEN_SECRET, SignatureAlgorithm.HS512).compact();
    ...
    public boolean validateToken(String token, HttpServletRequest httpServletRequest) {
        try {
            ...
            httpServletRequest.setAttribute("token_error", "Invalid JWT Signature");
        } catch (MalformedJwtException ex) {
            log.info("Invalid JWT token");
            httpServletRequest.setAttribute("token_error", "Invalid JWT token");
        } catch (ExpiredJwtException ex) {
            log.info("Expired JWT token");
            httpServletRequest.setAttribute("token_error", "Expired JWT token");
        } catch (UnsupportedJwtException ex) {
            log.info("Unsupported JWT exception");
            httpServletRequest.setAttribute("token_error", "Unsupported JWT token");
        } catch (IllegalArgumentException ex) {
            log.info("JWT claims string is empty");
            httpServletRequest.setAttribute("token_error", "JWT claims string is empty");
        }
        return false;
    }
