        ...
        if (jwtTokenHeader != null && securityContext.getAuthentication() == null) {
            ...
            if (tokenUtil.validateToken(jwtToken, request)) {
                ...
                if (username != null) {
                    ...
                    log.info("Authorities before setting authentication: " + userDetails.getAuthorities());
                    ...
                    if (tokenUtil.isTokenValid(jwtToken, userDetails)) {
                        ...
                        log.info("User authenticated and context set for " + username);
                        log.info("Security context after setting authentication: " + SecurityContextHolder.getContext().getAuthentication());
                    }
                }
            } else {
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Invalid Token");
//                log.error("dddd"+response.sendError(HttpServletResponse.SC_UNAUTHORIZED));
                return;
            }
        }
